...
def ensure_required_files():
    ...
    if not os.path.exists(TOOLS_FILE):
        print(f"{COLOR_YELLOW}[Warning]{COLOR_RESET} '{TOOLS_FILE}' not found. Creating a default version.")
        ...
        with open(TOOLS_FILE, 'w') as f:
            json.dump(default_tools_content, f, indent=4)


   # Check for and create 'model.functions.py' if it does not exist
    if not os.path.exists('model.functions.py'):
        print(f"{COLOR_YELLOW}[Warning]{COLOR_RESET} 'model.functions.py' not found. Creating a default version.")
        default_functions_content = '''\
# model.functions.py
...
'''
        with open('model.functions.py', 'w') as f:
            f.write(default_functions_content)
