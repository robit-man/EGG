...
#################################################################
#								  #
#  Change the folder structure to reflect where you place this  #
#  script and run it, also the shared folder 'container_shared' #
#								  #
#################################################################
...
start_container() {
    while true; do
        jetson-containers run -v /home/roko/container_shared:/container_shared $(autotag nano_llm) /bin/bash -c "
            while true; do
                python3 container_shared/simple.py 2>&1 | tee python_log.txt | while read -r line; do
                    echo \"\$line\"
                    if echo \"\$line\" | grep -q 'InternalError: Check failed: (offset + needed_size <= this->buffer.size)'; then
                        ...
                        pkill -f 'python3 container_shared/simple.py'
                        sleep 5
                        break
