...
# ---------------------------------------------------------------------------
AUDIO_VENV_DIR_NAME = "audio_router_venv"
AUDIO_CLOUDFLARED_BASENAME = "audio_router_cloudflared"
...
DEFAULT_ASR_DEVICE_ID = 0
DEFAULT_LLM_HOST = "127.0.0.1"
DEFAULT_LLM_PORT = 6545
DEFAULT_TTS_HOST = "127.0.0.1"
DEFAULT_TTS_PORT = 6434
DEFAULT_AUDIO_OUT_HOST = "127.0.0.1"
DEFAULT_AUDIO_OUT_PORT = 6353
DEFAULT_SOCKET_TIMEOUT_SECONDS = 8.0
...
def _integration_settings():
    with integration_runtime_lock:
        return dict(integration_runtime)
...
def _integration_status():
    cfg = _integration_settings()
    llm_ready = _tcp_ok(cfg["llm_host"], cfg["llm_port"])
    tts_ready = _tcp_ok(cfg["tts_host"], cfg["tts_port"])
    audio_out_ready = _tcp_ok(cfg["audio_out_host"], cfg["audio_out_port"])
    return {
        "llm_bridge": {
            "ready": llm_ready,
            "host": cfg["llm_host"],
            "port": int(cfg["llm_port"]),
        },
        "tts_server": {
            "ready": tts_ready,
            "host": cfg["tts_host"],
            "port": int(cfg["tts_port"]),
        },
        "audio_output": {
            "ready": audio_out_ready,
            "host": cfg["audio_out_host"],
            "port": int(cfg["audio_out_port"]),
        },
    }


def _send_llm_prompt(prompt):
    cfg = _integration_settings()
    timeout = float(cfg.get("socket_timeout_seconds", DEFAULT_SOCKET_TIMEOUT_SECONDS))
    payload = str(prompt or "").encode("utf-8", errors="replace")
    with socket.create_connection((cfg["llm_host"], int(cfg["llm_port"])), timeout=timeout) as sock:
        sock.sendall(payload)


def _send_tts_prompt(prompt):
    cfg = _integration_settings()
    timeout = float(cfg.get("socket_timeout_seconds", DEFAULT_SOCKET_TIMEOUT_SECONDS))
    payload = json.dumps({"prompt": str(prompt or "")}).encode("utf-8")
    with socket.create_connection((cfg["tts_host"], int(cfg["tts_port"])), timeout=timeout) as sock:
        sock.sendall(payload)
...
def _load_audio_settings(config):
    ...
    asr_device_id = _as_int(
        _read_config_value(
            config,
            "audio_router.audio.asr_device_id",
            DEFAULT_ASR_DEVICE_ID,
            legacy_keys=("asr_device_id", "input_device_id"),
        ),
        DEFAULT_ASR_DEVICE_ID,
        minimum=0,
        maximum=128,
    )
    promote("audio_router.audio.asr_device_id", asr_device_id)

    llm_host = str(
        _read_config_value(
            config,
            "audio_router.integrations.llm_host",
            DEFAULT_LLM_HOST,
        )
    ).strip() or DEFAULT_LLM_HOST
    promote("audio_router.integrations.llm_host", llm_host)

    llm_port = _as_int(
        _read_config_value(
            config,
            "audio_router.integrations.llm_port",
            DEFAULT_LLM_PORT,
        ),
        DEFAULT_LLM_PORT,
        minimum=1,
    ...
    promote("audio_router.integrations.llm_port", llm_port)

    tts_host = str(
        _read_config_value(
            config,
            "audio_router.integrations.tts_host",
            DEFAULT_TTS_HOST,
        )
    ).strip() or DEFAULT_TTS_HOST
    promote("audio_router.integrations.tts_host", tts_host)

    tts_port = _as_int(
        _read_config_value(
            config,
            "audio_router.integrations.tts_port",
            DEFAULT_TTS_PORT,
        ),
        DEFAULT_TTS_PORT,
        minimum=1,
    ...
    promote("audio_router.integrations.tts_port", tts_port)

    audio_out_host = str(
        _read_config_value(
            config,
            "audio_router.integrations.audio_out_host",
            DEFAULT_AUDIO_OUT_HOST,
        )
    ).strip() or DEFAULT_AUDIO_OUT_HOST
    promote("audio_router.integrations.audio_out_host", audio_out_host)

    audio_out_port = _as_int(
        _read_config_value(
            config,
            "audio_router.integrations.audio_out_port",
            DEFAULT_AUDIO_OUT_PORT,
        ),
        DEFAULT_AUDIO_OUT_PORT,
        minimum=1,
        maximum=65535,
    )
    promote("audio_router.integrations.audio_out_port", audio_out_port)

    socket_timeout_seconds = _as_float(
        _read_config_value(
            config,
            "audio_router.integrations.socket_timeout_seconds",
    ...
    promote("audio_router.integrations.socket_timeout_seconds", socket_timeout_seconds)

    prompt_max_chars = _as_int(
        _read_config_value(
            config,
            "audio_router.integrations.prompt_max_chars",
    ...
    promote("audio_router.integrations.prompt_max_chars", prompt_max_chars)
            ...
            CategorySpec(
                ...
                settings=(
                    ...
                    SettingSpec(
                        id="asr_device_id",
                        label="ASR Device ID",
                        path="audio_router.audio.asr_device_id",
                        value_type="int",
                        default=DEFAULT_ASR_DEVICE_ID,
                        min_value=0,
                        max_value=128,
                        description="Device index used by whisper stream capture.",
            ...
            CategorySpec(
                ...
                settings=(
                    SettingSpec(
                        id="llm_host",
                        label="LLM Host",
                        path="audio_router.integrations.llm_host",
                        value_type="str",
                        default=DEFAULT_LLM_HOST,
                        description="Socket host for llm prompt forwarding.",
                        restart_required=False,
                    ),
                    SettingSpec(
                        id="llm_port",
                        label="LLM Port",
                        path="audio_router.integrations.llm_port",
                        value_type="int",
                        default=DEFAULT_LLM_PORT,
                        min_value=1,
                        max_value=65535,
                        description="Socket port for llm prompt forwarding.",
                        restart_required=False,
                    ),
                    SettingSpec(
                        id="tts_host",
                        label="TTS Host",
                        path="audio_router.integrations.tts_host",
                        value_type="str",
                        default=DEFAULT_TTS_HOST,
                        description="Socket host for tts prompt forwarding.",
                        restart_required=False,
                    ),
                    SettingSpec(
                        id="tts_port",
                        label="TTS Port",
                        path="audio_router.integrations.tts_port",
                        value_type="int",
                        default=DEFAULT_TTS_PORT,
                        min_value=1,
                        max_value=65535,
                        description="Socket port for tts prompt forwarding.",
                        restart_required=False,
                    ),
                    SettingSpec(
                        id="audio_out_host",
                        label="Audio Out Host",
                        path="audio_router.integrations.audio_out_host",
                        value_type="str",
                        default=DEFAULT_AUDIO_OUT_HOST,
                        description="Health probe host for raw audio output service.",
                        restart_required=False,
                    ),
                    SettingSpec(
                        id="audio_out_port",
                        label="Audio Out Port",
                        path="audio_router.integrations.audio_out_port",
                        value_type="int",
                        default=DEFAULT_AUDIO_OUT_PORT,
                        min_value=1,
                        max_value=65535,
                        description="Health probe port for raw audio output service.",
                        restart_required=False,
                    ),
                    SettingSpec(
                        id="socket_timeout_seconds",
                        label="Socket Timeout",
                        path="audio_router.integrations.socket_timeout_seconds",
                        value_type="float",
                    ...
                    SettingSpec(
                        id="prompt_max_chars",
                        label="Prompt Max",
                        path="audio_router.integrations.prompt_max_chars",
                        value_type="int",
...
def apply_runtime_settings(settings):
    ...
    with audio_runtime_lock:
        ...
        audio_runtime["asr_device_id"] = int(settings["asr_device_id"])

    with integration_runtime_lock:
        integration_runtime["llm_host"] = str(settings["llm_host"]).strip() or DEFAULT_LLM_HOST
        integration_runtime["llm_port"] = int(settings["llm_port"])
        integration_runtime["tts_host"] = str(settings["tts_host"]).strip() or DEFAULT_TTS_HOST
        integration_runtime["tts_port"] = int(settings["tts_port"])
        integration_runtime["audio_out_host"] = str(settings["audio_out_host"]).strip() or DEFAULT_AUDIO_OUT_HOST
        integration_runtime["audio_out_port"] = int(settings["audio_out_port"])
        integration_runtime["socket_timeout_seconds"] = float(settings["socket_timeout_seconds"])
        integration_runtime["prompt_max_chars"] = int(settings["prompt_max_chars"])
    ...
    if ui:
        ...
        ui.update_metric("Prompt Max", str(integration_runtime["prompt_max_chars"]))
        ui.update_metric("Sock Timeout", f"{float(integration_runtime['socket_timeout_seconds']):.1f}s")
...
def current_audio_config():
    with audio_runtime_lock:
        return {
            ...
            "asr_device_id": int(audio_runtime.get("asr_device_id", DEFAULT_ASR_DEVICE_ID)),
        }
...
def _persist_audio_device_settings(input_value, output_value, asr_value=_MISSING):
    cfg = load_config()
    ...
    if asr_value is not _MISSING:
        try:
            parsed = int(asr_value)
        except Exception:
            parsed = DEFAULT_ASR_DEVICE_ID
        _set_nested(cfg, "audio_router.audio.asr_device_id", max(0, parsed))
    save_config(cfg)


def _set_runtime_audio_devices(input_value=_MISSING, output_value=_MISSING, asr_value=_MISSING):
    with audio_runtime_lock:
        if input_value is not _MISSING:
        ...
        if asr_value is not _MISSING:
            try:
                ...
                parsed = DEFAULT_ASR_DEVICE_ID
            audio_runtime["asr_device_id"] = max(0, parsed)
...
def _build_list_payload():
    ...
    stack = _integration_status()
    integration_cfg = _integration_settings()
    ...
    tunnel_base = str(current_tunnel or "").strip()
    ...
    return {
        ...
        "audio": {
            ...
            "asr_device_id": int(selection.get("asr_device_id", audio_runtime.get("asr_device_id", DEFAULT_ASR_DEVICE_ID))),
            "input_index": selection["input_index"],
            "output_index": selection["output_index"],
        ...
        "integrations": {
            "llm_host": integration_cfg["llm_host"],
            "llm_port": int(integration_cfg["llm_port"]),
            "tts_host": integration_cfg["tts_host"],
            "tts_port": int(integration_cfg["tts_port"]),
            "audio_out_host": integration_cfg["audio_out_host"],
            "audio_out_port": int(integration_cfg["audio_out_port"]),
            "socket_timeout_seconds": float(integration_cfg["socket_timeout_seconds"]),
            "prompt_max_chars": int(integration_cfg["prompt_max_chars"]),
        },
        "stack": stack,
        "devices": catalog,
        "protocols": {
        ...
        "endpoints": {
            ...
            "devices_url": f"{publish_base}/devices",
            "devices_select_url": f"{publish_base}/devices/select",
            "llm_prompt_url": f"{publish_base}/llm/prompt",
            "tts_speak_url": f"{publish_base}/tts/speak",
            "webrtc_offer_url": f"{publish_base}/webrtc/offer",
            ...
            "local_devices_url": f"{local_base}/devices",
            "local_devices_select_url": f"{local_base}/devices/select",
            "local_llm_prompt_url": f"{local_base}/llm/prompt",
            "local_tts_speak_url": f"{local_base}/tts/speak",
            "local_webrtc_offer_url": f"{local_base}/webrtc/offer",
            ...
            "lan_devices_url": f"{lan_base}/devices" if lan_base else "",
            "lan_devices_select_url": f"{lan_base}/devices/select" if lan_base else "",
            "lan_llm_prompt_url": f"{lan_base}/llm/prompt" if lan_base else "",
            "lan_tts_speak_url": f"{lan_base}/tts/speak" if lan_base else "",
            "lan_webrtc_offer_url": f"{lan_base}/webrtc/offer" if lan_base else "",
            ...
            "tunnel_list_url": f"{tunnel_base}/list" if tunnel_base else "",
            "tunnel_devices_url": f"{tunnel_base}/devices" if tunnel_base else "",
            "tunnel_devices_select_url": f"{tunnel_base}/devices/select" if tunnel_base else "",
            ...
            "tunnel_webrtc_offer_url": f"{tunnel_base}/webrtc/offer" if tunnel_base else "",
        },
        "routes": {
            "auth": "/auth",
            "session_rotate": "/session/rotate",
...
def select_devices():
    ...
    if "asr_device_id" in payload:
        asr_value = payload.get("asr_device_id")

    if input_value is _MISSING and output_value is _MISSING and asr_value is _MISSING:
        return jsonify(
            {
                "status": "error",
                "message": "Provide input_device, output_device, and/or asr_device_id",
            }
        ), 400

    _set_runtime_audio_devices(input_value=input_value, output_value=output_value, asr_value=asr_value)
    _persist_audio_device_settings(input_value, output_value, asr_value=asr_value)
...
def llm_prompt():
    ...
    cfg = _integration_settings()
    max_chars = int(cfg.get("prompt_max_chars", DEFAULT_PROMPT_MAX_CHARS))
    if len(prompt) > max_chars:
        return jsonify({"status": "error", "message": f"Prompt too long (max {max_chars})"}), 400
    try:
    ...
    return jsonify(
        {
            ...
            "ready": bool(
                stack.get("llm_bridge", {}).get("ready")
                and stack.get("tts_server", {}).get("ready")
                and stack.get("audio_output", {}).get("ready")
            ),
        ...
        return jsonify(
            {
                "status": "success",
                "tunnel_url": current_tunnel,
                "auth_url": f"{tunnel_base}/auth",
                "session_rotate_url": f"{tunnel_base}/session/rotate",
                "list_url": f"{tunnel_base}/list",
                "devices_url": f"{tunnel_base}/devices",
                "devices_select_url": f"{tunnel_base}/devices/select",
                "health_url": f"{tunnel_base}/health",
                "llm_prompt_url": f"{tunnel_base}/llm/prompt",
                "tts_speak_url": f"{tunnel_base}/tts/speak",
                "webrtc_offer_url": f"{tunnel_base}/webrtc/offer",
...
def router_info():
    ...
    selected_base = current_tunnel or (upnp_base if selected_transport == "upnp" else "") or publish_base
    ...
    return jsonify(
        {
            ...
            "tunnel": {
                "state": tunnel_state,
                "tunnel_url": current_tunnel,
                "auth_url": f"{current_tunnel}/auth" if current_tunnel else "",
                "session_rotate_url": f"{current_tunnel}/session/rotate" if current_tunnel else "",
                "list_url": f"{current_tunnel}/list" if current_tunnel else "",
                "devices_url": f"{current_tunnel}/devices" if current_tunnel else "",
                "devices_select_url": f"{current_tunnel}/devices/select" if current_tunnel else "",
                "health_url": f"{current_tunnel}/health" if current_tunnel else "",
                "llm_prompt_url": f"{current_tunnel}/llm/prompt" if current_tunnel else "",
                "tts_speak_url": f"{current_tunnel}/tts/speak" if current_tunnel else "",
...
def metrics_update_loop():
    while ui and ui.running:
        ...
        ui.update_metric("ASR Device", str(selection.get("asr_device_id", DEFAULT_ASR_DEVICE_ID)))
        ui.update_metric("LLM", "ready" if stack.get("llm_bridge", {}).get("ready") else "waiting")
        ui.update_metric("TTS", "ready" if stack.get("tts_server", {}).get("ready") else "waiting")
        ui.update_metric("Audio Out", "ready" if stack.get("audio_output", {}).get("ready") else "waiting")
        ui.update_metric("Prompt Max", str(integration_cfg.get("prompt_max_chars", DEFAULT_PROMPT_MAX_CHARS)))
        ui.update_metric(
            "Sock Timeout",
            f"{float(integration_cfg.get('socket_timeout_seconds', DEFAULT_SOCKET_TIMEOUT_SECONDS)):.1f}s",
        )
...
def main():
    ...
    if ui:
        ...
        ui.update_metric("Auth", "Required" if runtime_security["require_auth"] else "Disabled")
        ui.update_metric("Session Timeout", str(SESSION_TIMEOUT))
        ui.update_metric("ASR Device", str(settings.get("asr_device_id", DEFAULT_ASR_DEVICE_ID)))
        ui.update_metric("Prompt Max", str(integration_cfg.get("prompt_max_chars", DEFAULT_PROMPT_MAX_CHARS)))
        ui.update_metric(
            "Sock Timeout",
            f"{float(integration_cfg.get('socket_timeout_seconds', DEFAULT_SOCKET_TIMEOUT_SECONDS)):.1f}s",
