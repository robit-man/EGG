...
def build_resolved_endpoints(services):
    ...
    adapter_llm_prompt = str(
        adapter_tunnel.get("llm_prompt_url")
        or adapter_local.get("llm_prompt_url")
        or (f"{adapter_base}/llm/prompt" if adapter_base else "")
    ).strip()
    adapter_tts_speak = str(
        adapter_tunnel.get("tts_speak_url")
        or adapter_local.get("tts_speak_url")
        or (f"{adapter_base}/tts/speak" if adapter_base else "")
    ).strip()
    ...
    return {
        "adapter": {
            ...
            "llm_prompt_url": adapter_llm_prompt,
            "tts_speak_url": adapter_tts_speak,
        },
        "camera": {
            ...
            "video_url": str(camera_tunnel.get("video_url") or (f"{camera_base}/video/cam0" if camera_base else "")).strip(),
            "snapshot_url": str(
                camera_tunnel.get("snapshot_url") or (f"{camera_base}/snapshot/cam0" if camera_base else "")
            ).strip(),
        ...
        "audio": {
            ...
            "llm_prompt_url": str(
                audio_tunnel.get("llm_prompt_url")
                or audio_local.get("llm_prompt_url")
                or (f"{audio_base}/llm/prompt" if audio_base else "")
            ).strip(),
            "tts_speak_url": str(
                audio_tunnel.get("tts_speak_url")
                or audio_local.get("tts_speak_url")
                or (f"{audio_base}/tts/speak" if audio_base else "")
            ).strip(),
